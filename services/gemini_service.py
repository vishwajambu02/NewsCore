...
def translate_fields_batch(items: list, target_language: str):
    ...
    `items` is a list of dicts like {'title':..., 'summary':...}.

    Returns (merged_list, translated_indices):
      - merged_list is the same order/length as items, each merged with
        whatever translated values came back (falls back to the original
