...
def _apply(article, translated):
    ...
    # CRITICAL: detach from the DB session immediately after mutating.
    # These fields are display-only translations — if we leave the object
    # attached, ANY later query in the same request (e.g. the admin-flag
    # check on every page) triggers SQLAlchemy autoflush, which tries to
    # UPDATE the real articles row with the translated text — permanently
    # overwriting the original English content in the database.
    try:
        if article in db.session:
